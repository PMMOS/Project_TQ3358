...
public class FunSupport implements IFunSDKResult {
	...
	private static final String APP_UUID = "004a052a9b124d2bb0a828fa7424ad01";
	private static final String APP_KEY = "4368d207c05a4f799cc6100414c56dd2";
	private static final String APP_SECRET = "9bc94b22df164f26aaf841e8d634ed7d";
	...
	public void init(Context context) {
		...
		FunSDK.XMCloundPlatformInit(APP_UUID, // uuid
				APP_KEY, // App Key
	...
	public void setAutoLogin(boolean auto) {
		...
		mSharedParam.setBooleanUserValue(SHARED_PARAM_KEY_AUTOLOGIN, auto);
	}
	...
	public void setSaveNativePassword(boolean is) {
		...
		mSharedParam.setBooleanUserValue(SHARED_PARAM_KEY_SAVENATIVEPASSWORD, is);
	}
	...
	public void setSavePasswordAfterLogin(boolean save) {
		...
		mSharedParam.setBooleanUserValue(SHARED_PARAM_KEY_SAVEPASSWORD, save);
	}
	...
	public boolean loginByLastUser() {
		...
		if (null != lastUserName && lastUserName.length() > 0 && null != lastPassWord && lastPassWord.length() > 0) {
	...
	public boolean login(String username, String password) {
		...
		// int result = FunSDK.SysLoginToXM(getHandler(), username, password,
		// 0);
		// return (result == 0);

		// 用户获取设备列表来替代用户登录
		int result = FunSDK.SysGetDevList(getHandler(), mTmpLoginUserName, mTmpLoginPassword, 0);
		return (result == 0);
	}
	...
	public boolean hasLogin() {
		if (null != mLoginUserName && mLoginUserName.length() > 0 && null != mLoginPassword
	 ...
	 * @return
	 */
	public boolean registerByPhone(String userName, String passWd, String checkCode, String phoneNo) {
		int result = FunSDK.SysRegUserToXM(getHandler(), userName, passWd, checkCode, phoneNo, 0);
		return (result == 0);
	}
	 ...
	 * @return
	 */
	public boolean registerByEmail(String userName, String passWd, String code, String email) {
		int result = FunSDK.SysRegisteByEmail(getHandler(), userName, passWd, email, code, 0);
		return (result == 0);
	}
	...
	public boolean changePassw(String userName, String oldPassw, String newPassw) {
		// int result = FunSDK.SysPswChange(getHandler(), userName, oldPassw,
		// newPassw, 0);
	...
	public boolean requestDeviceList() {
		...
		int result = FunSDK.SysGetDevList(getHandler(), mLoginUserName, mLoginPassword, 0);
		return (result == 0);
	}
	...
	public boolean requestDeviceRename(FunDevice funDevice, String newDevName) {
		...
		int result = FunSDK.SysChangeDevInfo(getHandler(), G.ObjToBytes(devInfo), mLoginUserName, mLoginPassword,
				funDevice.getId());
		return (result == 0);
	}
	...
	public boolean requestDeviceRemove(FunDevice funDevice) {
		int result = FunSDK.SysDeleteDev(getHandler(), funDevice.devMac, mLoginUserName, mLoginPassword, 0);
		return (result == 0);
	}
	...
	public boolean requestDeviceAdd(FunDevice funDevice) {
		...
		int result = FunSDK.SysAddDevice(getHandler(), G.ObjToBytes(devInfo), mLoginUserName, mLoginPassword, 0);
		return (result == 0);
	}
		...
		new Thread() {
			...
			public void run() {
				...
				for (int i = 0; i < scanSecond; i++) {
					...
					DeviceWifiManager.getInstance(getContext()).startScan(DeviceWifiManager.WIFI_TYPE.DEV_AP,
							ONE_SECOND);
					...
					for (ScanResult scanResult : scanResults) {
						...
						if (null != funDevice) {
							...
							// SDBDeviceInfo devInf =
							// funDevice.toSDBDeviceInfo();
							// FunLog.i("test", "AddDevice:" +
							// devInf.toString());
	 ...
	 * @return
	 */
	public boolean startWiFiQuickConfig(String ssid, String data, String info, String gw_ipaddr, int type, int isbroad,
			String mac, int nTimeout) {
		int nGetType = 2; // 1:外网;2:内网 0x3:一起
		int result = FunSDK.DevStartAPConfig(getHandler(), nGetType, ssid, data, info, gw_ipaddr, type, isbroad, mac,
				nTimeout);
		return (result == 0);
	}
	...
	public boolean transportSerialOpen(FunDevice funDevice) {
		int result = FunSDK.DevOption(getHandler(), // userId句柄
				...
				null, 0, SDK_CommTypes.SDK_COMM_TYPES_RS485, 0, 0, "COM_OPEN", funDevice.getId());
		return (result == 0);
	}
	...
	public boolean transportSerialClose(FunDevice funDevice) {
		int result = FunSDK.DevOption(getHandler(), funDevice.getDevSn(), EDEV_OPTERATE.EDOPT_DEV_CLOSE_TANSPORT_COM,
				null, 0, SDK_CommTypes.SDK_COMM_TYPES_RS485, 0, 0, "COM_CLOSE", funDevice.getId());
		return (result == 0);
	}
	...
	public boolean transportSerialWrite(FunDevice funDevice, byte[] pData) {
		int result = FunSDK.DevOption(getHandler(), funDevice.getDevSn(), EDEV_OPTERATE.EDOPT_DEV_TANSPORT_COM_WRITE,
				pData, pData.length, SDK_CommTypes.SDK_COMM_TYPES_RS485, 0, 0, "COM_WRITE", funDevice.getId());
		return (result == 0);
	}
	...
	public boolean requestDeviceLogin(FunDevice funDevice) {
		...
		// }
		// String loginPsd = (null == funDevice.NativeLoginPsw) ? "" :
		// funDevice.NativeLoginPsw;
		...
		int result = FunSDK.DevLogin(getHandler(), funDevice.getDevSn(), loginName, loginPsd, funDevice.getId());
	 ...
	 * @return
	 */
	public boolean requestDeviceLogin(String devMac, String loginName, String loginPasswd) {
		...
		int result = FunSDK.DevLogin(getHandler(), devMac, loginName, loginPasswd, funDevice.getId());
		return (result == 0);
	}
	...
	public boolean requestDeviceLogout(FunDevice funDevice) {
		int result = FunSDK.DevLogout(getHandler(), funDevice.getDevSn(), funDevice.getId());
	...
	public boolean requestDeviceStatus(FunDevice funDevice) {
		...
		int result = FunSDK.SysGetDevState(getHandler(), funDevice.getDevSn(), funDevice.getId());
		return (result == 0);
	}
	...
	public boolean requestDeviceConfig(FunDevice funDevice) {
		...
		int result = FunSDK.DevGetConfig(getHandler(), funDevice.getDevSn(), ECONFIG.CFG_ATHORITY, G.Sizeof(authority),
	...
	public boolean requestDeviceConfig(FunDevice funDevice, String configName) {
		// int result = FunSDK.DevGetConfigByJson(getHandler(),
		// funDevice.getDevSn(),
		...
		// mDeviceGetConfigQueue.add(new DeviceGetConfig(funDevice,
		// configName));
		// }
	...
	private boolean requestDeviceConfigFromQueue() {
		...
		if (null != devCfg) {
			int result = FunSDK.DevGetConfigByJson(getHandler(), devCfg.funDevice.getDevSn(), devCfg.configName, 4096,
					devCfg.channelNo, 10000, devCfg.funDevice.getId());
	...
	private void requestDeviceConfigDone(FunDevice funDevice, String configName) {
		...
		synchronized (mDeviceGetConfigQueue) {
			...
			for (int i = 0; i < mDeviceGetConfigQueue.size(); i++) {
				...
				if (tmpCfg.configName.equals(configName) && tmpCfg.funDevice.getId() == funDevice.getId()) {
					devCfg = tmpCfg;
		...
		if (devCfg == null) {
			...
			FunLog.e(TAG,
					"Error!!! must be the same configName here! [" + devCfg.configName + "] != [" + configName + "]");
		} else {
			synchronized (mDeviceGetConfigQueue) {
				mIsGettingConfig = false;
			}
	...
	public boolean requestDeviceSetConfig(FunDevice funDevice, Object parmObj) {
		...
		if (parmObj instanceof BaseConfig) {
			...
			result = FunSDK.DevSetConfigByJson(getHandler(), funDevice.getDevSn(), baseConfig.getConfigName(),
					baseConfig.getSendMsg(), funDevice.CurrChannel, 60000, funDevice.getId());
		}
	...
	public boolean requestDeviceCmdGeneral(FunDevice funDevice, DevCmdGeneral cmd) {
		int result = FunSDK.DevCmdGeneral(getHandler(), funDevice.getDevSn(), cmd.getJsonID(), cmd.getConfigName(), 0,
				10000, cmd.getSendMsg().getBytes(), -1, funDevice.getId());
		return (result == 0);
	}

	public boolean requestDeviceFileNum(FunDevice funDevice, DevCmdSearchFileNumJP cmd, int nSeq) {
		int result = FunSDK.DevCmdGeneral(getHandler(), funDevice.getDevSn(), cmd.getJsonID(), cmd.getConfigName(),
				1024, 10000, cmd.getSendMsg().getBytes(), -1, nSeq);
		return (result == 0);
	}

	public boolean requestDeviceFileList(FunDevice funDevice, H264_DVR_FINDINFO info) {
		int result = FunSDK.DevFindFile(getHandler(), funDevice.getDevSn(), G.ObjToBytes(info), 256, 20000,
				funDevice.getId());
		Log.i("SDK_LOG", "--> DevFindFile : info = " + info.toString());
		return (result == 0);
	}

	public boolean requestDeviceFileListByTime(FunDevice funDevice, SDK_SearchByTime info) {
		int result = FunSDK.DevFindFileByTime(getHandler(), funDevice.getDevSn(), G.ObjToBytes(info), 10000, 0);
		Log.i("SDK_LOG", "--> DevFindFileByTime : info = " + info.toString());
		return (result == 0);
	}

	public boolean requestDeviceSearchPicture(FunDevice funDevice, OPCompressPic opCompressPic, String filePath,
			int seq) {
		int result = FunSDK.DevSearchPicture(getHandler(), funDevice.getDevSn(), EDEV_JSON_ID.COMPRESS_PICTURE_REQ,
				50000, 2000, opCompressPic.getSendMsg().getBytes(), 20, -1, filePath, seq);
		return (result == 0);
	}

	public boolean requestDeviceRemovePicture(FunDevice funDevice, DevCmdOPRemoveFileJP cmdOPRemoveFileJP) {
		int result = FunSDK.DevCmdGeneral(getHandler(), funDevice.getDevSn(), cmdOPRemoveFileJP.getJsonID(),
				cmdOPRemoveFileJP.getConfigName(), cmdOPRemoveFileJP.getFileNum() * 256,
				cmdOPRemoveFileJP.getFileNum() * 2000, cmdOPRemoveFileJP.getSendMsg().getBytes(), -1, 0);
		return (result == 0);
	}

	public boolean requestDeviceSearchPicByPath(FunDevice funDevice, DevCmdOPFileQueryJP cmdOPFileQueryJP, int seq) {
		int result = FunSDK.DevCmdGeneral(getHandler(), funDevice.getDevSn(), cmdOPFileQueryJP.getJsonID(),
				cmdOPFileQueryJP.getConfigName(), -1, 1000, cmdOPFileQueryJP.getSendMsg().getBytes(), -1, seq);
	 ...
	 * @return
	 */
	public boolean requestDeviceSearchAlarmInfo(FunDevice funDevice, XPMS_SEARCH_ALARMINFO_REQ info) {
		int result = MpsClient.SearchAlarmInfo(getHandler(), G.ObjToBytes(info), funDevice.getId());
		return (result == 1);
	}
	...
	public boolean requestDeviceCapture(FunDevice funDevice) {
		...
		int result = FunSDK.DevOption(getHandler(), funDevice.getDevSn(), EDEV_OPTERATE.EDOPT_DEV_GET_IMAGE, null, 0, 0,
				0, 0, FunPath.PATH_CAPTURE_TEMP + File.separator + strDate + ".jpg", funDevice.getId());
		return (result == 1);
	 ...
	 * @param funDevices
	 *            目标设备
	 * @param pH264_DVR_FILE_DATA
	 *            查询条件。H264_DVR_FILE_DATA 对象字节流
	 * @param szFilePath
	 *            保存文件路径+文件名
	 * @return
	 */
	public boolean requestDeviceDownloadByFile(FunDevice funDevices, byte[] pH264_DVR_FILE_DATA, String szFilePath,
			int position) {
		int result = FunSDK.DevDowonLoadByFile(getHandler(), funDevices.getDevSn(), pH264_DVR_FILE_DATA, szFilePath,
				position);
		return (result == 1);
	}

	/**
	 * EMSG_ON_FILE_DOWNLOAD: 请求设备开始对讲
	 * 
	 * @param funDevice
	 * @return
	 */
	 ...
	 * @param funDevice
	 * @param nPTZCommand
	 *            参考EPTZCMD.TILT_UP,EPTZCMD.TILT_DOWN,EPTZCMD.PAN_RIGHT,EPTZCMD.
	 *            PAN_LEFT
	 * @param bStop
	 * @return
	 */
	public boolean requestDevicePTZControl(FunDevice funDevice, int nPTZCommand, boolean bStop) {
		int result = FunSDK.DevPTZControl(getHandler(), funDevice.getDevSn(), 0, nPTZCommand, bStop ? 1 : 0, 4,
				funDevice.getId());
		return (result == 1);
	}
	 ...
	 * @return
	 */
	public boolean requestDeviceLanAlarmEnable(String devSn, boolean enable) {
		int nValue = enable ? 1 : 0; // 0为关闭 ，非0为开启（建议赋1）
		int result = FunSDK.DevSetAttrAlarmByInt(getHandler(), devSn, EDEV_ATTR.EDA_OPT_ALARM, nValue, 0);
		return (result == 1);
	}
	...
	public boolean requestDeviceTitleDot(FunDevice funDevice, SDK_TitleDot titleDot) {
		int result = FunSDK.DevCmdGeneral(getHandler(), funDevice.getDevSn(), EDEV_JSON_ID.CONFIG_CHANNELTILE_DOT_SET,
				"TitleDot", 1024, 5000, G.ObjToBytes(titleDot), -1, 0);
		return (result == 0);
	}
	...
	private void mpsInit() {
		// 初始化消息推送服务
		try {
			if (null != mLoginUserName && !mLoginUserName.equals(mLastMpsUserName)) {
	...
	public void mpsUnLinkDevice(String devSn) {
		if (null != devSn) {
			MpsClient.UnlinkDev(getHandler(), devSn, 0);
		}
	}
	...
	public int OnFunSDKResult(Message msg, MsgContent msgContent) {
		...
		case EUIMSG.SYS_GET_DEV_STATE: // 获取设备在线状态
		{
			...
			if (null != funDev) {
				// 判断离线/在线状态
				if (funDev.devStatus != devStatus || funDev.devStatusValue != msg.arg1) {
		...
		case EUIMSG.DEV_CMD_EN: {
			...
			if (null != funDevice) {
				...
				if (msg.arg1 < 0) {
					for (OnFunListener l : mListeners) {
						if (l instanceof OnFunDeviceOptListener) {
							((OnFunDeviceOptListener) l).onDeviceGetConfigFailed(funDevice, msg.arg1);
						}
					}
					...
					if (DeviceGetJson.onParse(funDevice, msgContent.str, json)) {
						...
						for (OnFunListener l : mListeners) {
							if (l instanceof OnFunDeviceOptListener) {
								((OnFunDeviceOptListener) l).onDeviceGetConfigSuccess(funDevice, msgContent.str,
										msgContent.seq);
							}
						}
					} else {
						for (OnFunListener l : mListeners) {
							if (l instanceof OnFunDeviceOptListener) {
								((OnFunDeviceOptListener) l).onDeviceGetConfigFailed(funDevice,
		...
		case EUIMSG.DEV_SET_JSON: {
			...
			if (null != funDevice) {
				if (msg.arg1 < 0) {
					for (OnFunListener l : mListeners) {
						if (l instanceof OnFunDeviceOptListener) {
							((OnFunDeviceOptListener) l).onDeviceSetConfigFailed(funDevice, msgContent.str, msg.arg1);
						}
					}
				} else {
					for (OnFunListener l : mListeners) {
						if (l instanceof OnFunDeviceOptListener) {
							((OnFunDeviceOptListener) l).onDeviceSetConfigSuccess(funDevice, msgContent.str);
						}
					}
				}
			} else {
		...
		case EUIMSG.DEV_OPTION: {
			...
			if (EDEV_OPTERATE.EDOPT_DEV_GET_IMAGE == msgContent.arg3) {
				...
				if (msg.arg1 < 0) {
					for (OnFunListener l : mListeners) {
						if (l instanceof OnFunDeviceSerialListener) {
							((OnFunDeviceSerialListener) l).onDeviceSerialOpenFailed(funDevice, msg.arg1);
						}
					}
				} else {
					for (OnFunListener l : mListeners) {
						if (l instanceof OnFunDeviceSerialListener) {
							((OnFunDeviceSerialListener) l).onDeviceSerialOpenSuccess(funDevice);
						}
					}
				}
				...
				if (msg.arg1 < 0) {
					for (OnFunListener l : mListeners) {
						if (l instanceof OnFunDeviceSerialListener) {
							((OnFunDeviceSerialListener) l).onDeviceSerialWriteFailed(funDevice, msg.arg1);
						}
					}
				} else {
					for (OnFunListener l : mListeners) {
						if (l instanceof OnFunDeviceSerialListener) {
							((OnFunDeviceSerialListener) l).onDeviceSerialWriteSuccess(funDevice);
						}
					}
				}
			} else {
				if (null != funDevice) {
					if (msg.arg1 < 0) {
						for (OnFunListener l : mListeners) {
							if (l instanceof OnFunDeviceOptListener) {
								((OnFunDeviceOptListener) l).onDeviceOptionFailed(funDevice, msgContent.str, msg.arg1);
							}
						}
					} else {
						for (OnFunListener l : mListeners) {
							if (l instanceof OnFunDeviceOptListener) {
								((OnFunDeviceOptListener) l).onDeviceOptionSuccess(funDevice, msgContent.str);
							}
						}
					}
				}
		...
		case EUIMSG.DEV_FIND_FILE: // 设备文件信息搜索
		{
			...
			if (null != funDevice && fileNum > 0 && null != msgContent.pData && null != cmdCalendar) {
			...
			if (msg.arg1 == FunError.EE_OK) {
				...
				for (OnFunListener l : mListeners) {
					if (l instanceof OnFunDeviceFileListener) {
						((OnFunDeviceFileListener) l).onDeviceFileDownCompleted(mCurrDevice, path, msgContent.seq);
					}
				}
			}
		}
		...
		case EUIMSG.ON_FILE_DLD_COMPLETE: {
			...
			for (OnFunListener l : mListeners) {
				if (l instanceof OnFunDeviceFileListener) {
					((OnFunDeviceFileListener) l).onDeviceFileDownCompleted(mCurrDevice, msgContent.str,
							msgContent.seq);
				}
			}
		}
		...
		case EUIMSG.ON_FILE_DLD_POS: {
			...
			for (OnFunListener l : mListeners) {
				if (l instanceof OnFunDeviceFileListener) {
					((OnFunDeviceFileListener) l).onDeviceFileDownProgress(msg.arg1, msg.arg2, msgContent.seq);
				}
			}
		}
			break;
		...
		case EUIMSG.ON_FILE_DOWNLOAD: {
			...
			for (OnFunListener l : mListeners) {
				if (l instanceof OnFunDeviceFileListener) {
					((OnFunDeviceFileListener) l).onDeviceFileDownStart(msg.arg1 == 0, msgContent.seq);
				}
			}
		}
			break;
		...
		case EUIMSG.DEV_GET_LAN_ALARM: {
			...
			if (null != funDevice) {
				try {
					...
					for (OnFunListener l : mListeners) {
						if (l instanceof OnFunDeviceAlarmListener) {
							((OnFunDeviceAlarmListener) l).onDeviceLanAlarmReceived(funDevice, alarmInfo);
						}
					}
				} catch (Exception e) {
					e.printStackTrace();
